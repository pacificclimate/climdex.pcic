...
valid.climdexInput <- function(object) {
  ...
  data.lengths <- c(sapply(object@data, length), sapply(length.check.slots, function(y) length(slot(object, y))), unlist(sapply(length.check.members, function(y) {
    sapply(slot(object, y), length)
  })))
...
get.date.field <- function(input.data, cal, date.types) {
  valid.date.types <- sapply(date.types, function(x) {
    return(!inherits(try(input.data[, x$fields], silent = TRUE), "try-error"))
  })
...
check.quantile.validity <- function(quantiles, present.vars, days.in.base) {
  ...
  if (!all(sapply(quantiles[names(quantiles) %in% intersect(present.vars, c("tmax", "tmin"))], function(x) {
    "outbase" %in% names(x) && all(c("q10", "q90") %in% names(x$outbase))
  }))) {
    stop("Temperature out-of-base quantiles must contain 10th and 90th percentiles.\n")
  }

  if (any(days.in.base > 0) && !all(sapply(quantiles[names(quantiles) %in% intersect(intersect(present.vars, c("tmax", "tmin")), names(days.in.base)[days.in.base > 0])], function(x) {
    "inbase" %in% names(x) && all(c("q10", "q90") %in% names(x$inbase))
  }))) {
...
climdex.get.available.indices <- function(ci, function.names = TRUE) {
  available.indices <- list(
    tmax = c("su", "id", "txx", "txn", "tx10p", "tx90p", "wsdi"),
