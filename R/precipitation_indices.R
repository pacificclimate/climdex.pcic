...
#'
#' @export
climdex.r10mm <- function(ci) {
  stopifnot(!is.null(ci@data$prec))
  return(number.days.op.threshold(ci@data$prec, ci@date.factors$annual, 10, ">=") * ci@namasks$annual$prec)
}
...
#'
#' @export
climdex.r20mm <- function(ci) {
  stopifnot(!is.null(ci@data$prec))
  return(number.days.op.threshold(ci@data$prec, ci@date.factors$annual, 20, ">=") * ci@namasks$annual$prec)
}
...
#'
#' @export
climdex.prcptot <- function(ci) {
  stopifnot(!is.null(ci@data$prec))
  return(total.precip.op.threshold(ci@data$prec, ci@date.factors$annual, 1, ">=") * ci@namasks$annual$prec)
}
