...
#'
#' @export
climdex.su <- function(ci) {
  stopifnot(!is.null(ci@data$tmax))
  return(number.days.op.threshold(ci@data$tmax, ci@date.factors$annual, 25, ">") * ci@namasks$annual$tmax)
}
...
#'
#' @export
climdex.id <- function(ci) {
  stopifnot(!is.null(ci@data$tmax))
  return(number.days.op.threshold(ci@data$tmax, ci@date.factors$annual, 0, "<") * ci@namasks$annual$tmax)
}
...
#'
#' @export
climdex.tr <- function(ci) {
  stopifnot(!is.null(ci@data$tmin))
  return(number.days.op.threshold(ci@data$tmin, ci@date.factors$annual, 20, ">") * ci@namasks$annual$tmin)
}
...
climdex.wsdi <- function(ci, spells.can.span.years = FALSE) {
  ...
  return(threshold.exceedance.duration.index(ci@data$tmax, ci@date.factors$annual, ci@jdays, ci@quantiles$tmax$outbase$q90, ">", spells.can.span.years = spells.can.span.years, max.missing.days = ci@max.missing.days["annual"]) * ci@namasks$annual$tmax)
}
...
climdex.csdi <- function(ci, spells.can.span.years = FALSE) {
  ...
  return(threshold.exceedance.duration.index(ci@data$tmin, ci@date.factors$annual, ci@jdays, ci@quantiles$tmin$outbase$q10, "<", spells.can.span.years = spells.can.span.years, max.missing.days = ci@max.missing.days["annual"]) * ci@namasks$annual$tmin)
}
...
#' tmax.dates <- as.PCICt(do.call(paste, ec.1018935.tmax[, c(
#'   "year",
#'   "jday"
#' )]), format = "%Y %j", cal = "gregorian")
#' tmin.dates <- as.PCICt(do.call(paste, ec.1018935.tmin[, c(
#'   "year",
#'   "jday"
#' )]), format = "%Y %j", cal = "gregorian")
#' prec.dates <- as.PCICt(do.call(paste, ec.1018935.prec[, c(
#'   "year",
#'   "jday"
#' )]), format = "%Y %j", cal = "gregorian")
#'
    ...
    growing.season <- (tapply.fast(1:length(daily.mean.temp), date.factor, function(idx) {
      ...
      if (include.exact.dates) {
        if (northern.hemisphere) {
          ...
          origin <- paste(year = unique(date.factor[idx]), "07", "01", sep = "-")
        }
