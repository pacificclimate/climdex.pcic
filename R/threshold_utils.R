...
#'   ci@data$tmax, ci@dates, ci@jdays,
...
#' alttedi <- threshold.exceedance.duration.index(
#'   prec.dat,
#'   phony.date.factor, rep(1:5, 2), rep(1, 5), ">=", 2, TRUE, 1
#' )
#'
#' ## Without spells spanning years...
#' tedi <- threshold.exceedance.duration.index(
#'   prec.dat, phony.date.factor,
...
threshold.exceedance.duration.index <- function(daily.temp, date.factor, jdays, thresholds, op = ">", min.length = 6, spells.can.span.years = TRUE, max.missing.days) {
  stopifnot(
    is.numeric(c(daily.temp, thresholds, min.length)), is.factor(date.factor),
