...
climdex.pcic.test.compute.stat.vector.circular.mean <- function() {
  ...
  checkEqualsNumeric(as.numeric(result_with_na$direction[1]), 350,
    tolerance = 1e-6,
...
climdex.pcic.test.compute.stat.vector.circular.sd <- function() {
  ...
  checkEqualsNumeric(as.numeric(result_opposite_direction$circular_sd[1]), expected_sd_opp,
    tolerance = 1e-6,
...
climdex.pcic.test.compute.stat.vector.overlapping.years <- function() {
  ...
  checkEqualsNumeric(as.numeric(result$magnitude["Winter 2019"]), expected_mean_speed,
    msg = paste(
      "Computed seasonal mean:", result$magnitude[1],
...
climdex.pcic.test.compute.stat.vector.cartesian.format <- function() {
  ...
  checkEqualsNumeric(as.numeric(result$magnitude[1]), expected_mean_magnitude,
    tolerance = 1e-6,
    msg = paste(
      "Computed mean magnitude:", result$magnitude[1],
  ...
  checkEqualsNumeric(as.numeric(result_circular_mean$direction[1]), expected_circular_mean,
    tolerance = 1e-6,
    msg = paste(
      "Computed circular mean direction:", result_circular_mean$direction[1],
