...
# Test for scalar with missing value in data
...
# Test for vector with missing value in primary data
...
climdex.pcic.test.MultiyearScalarContinuous <- function() {
  ...
  checkEquals(length(scalar_obj@data), length(scalar_obj@dates), "Multiyear scalar data length does not match dates.")
...
climdex.pcic.test.SingleMonthlyScalar.raw.NA.dates <- function() {
  ...
  # Expect an error due to NA in dates
  checkException(
    climdexSingleMonthlyScalar.raw(data, dates, northern.hemisphere = TRUE, calendar = "gregorian"),
    "Function did not raise an error when NA values were present in dates."
  )
}
...
climdex.pcic.test.SingleMonthlyVector.raw.missing.secondary <- function() {
  ...
  result <- try(
    climdexSingleMonthlyVector.raw(primary, secondary, dates, format = "polar", northern.hemisphere = TRUE, calendar = "gregorian"),
    silent = TRUE
  )
