...
climdex.pcic.test.scalar.exact.dates <- function() {
  ...
  checkEqualsNumeric(as.numeric(result$val[1]), expected_max,
    msg = paste(
      "Computed max statistic:", result$val[1],
  ...
  checkEquals(computed_exact_date, expected_max_date,
    msg = paste(
      "Computed exact date:", computed_exact_date,
...
climdex.pcic.test.compute.stat.scalar.sd <- function() {
  ...
  expected_sd <- sd(data)
  checkEqualsNumeric(as.numeric(result[1]), expected_sd,
    tolerance = 1e-6,
...
climdex.pcic.test.compute.stat.scalar.var <- function() {
  ...
  checkEqualsNumeric(as.numeric(result[1]), expected_var,
    tolerance = 1e-6,
