...
climdex.pcic.test.growing.season.length <- function() {
  ...
  res <- sapply(num.days.set, function(num.days) {
    sapply(td, function(dat) {
      sapply(modes, function(mode) {
        growing.season.length(dat, test.dates.factor, test.dates, TRUE, num.days, test.threshold, mode)
      }, simplify = "array")
    }, simplify = "array")
  }, simplify = "array")
...
climdex.pcic.test.bootstrap.quantiles <- function() {
  ...
  check.inbase.quantiles.for.day <- function(ci, pctile, var, day) {
    ...
    comparison.data <- t(sapply(1:nyr, function(x) {
  ...
  check.outbase.quantiles <- function(ci, pctile, var) {
    ...
    comparison.data <- sapply(1:length(climdex.pcic.data), function(x) {
